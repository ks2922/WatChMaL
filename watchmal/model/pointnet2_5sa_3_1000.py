...
class PointNet2(nn.Module):
    """PointNet++ 5SA Version 3, scaled for n_points=1000."""
    def __init__(self, num_input_channels, num_output_channels, dropout=0.3, **kwargs):
        super().__init__()
        in_feat = num_input_channels - 3
        self.SA_modules = nn.ModuleList([
            PointnetSAModule(npoint=1000, radius=30.0, nsample=32,
                mlp=[in_feat, 32, 32, 64], use_xyz=True),
            PointnetSAModule(npoint=500, radius=80.0, nsample=32,
                mlp=[64, 64, 64, 128], use_xyz=True),
            PointnetSAModule(npoint=250, radius=130.0, nsample=32,
                mlp=[128, 128, 128, 256], use_xyz=True),
            PointnetSAModule(npoint=63, radius=180.0, nsample=32,
