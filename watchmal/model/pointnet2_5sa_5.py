...
class PointNet2(nn.Module):
    """PointNet++ 5SA Version 5 - aggressive radius, conservative npoint."""
    def __init__(self, num_input_channels, num_output_channels, dropout=0.3, **kwargs):
        super().__init__()
        in_feat = num_input_channels - 3

        self.SA_modules = nn.ModuleList([
            PointnetSAModule(npoint=512, radius=40.0, nsample=32,
                mlp=[in_feat, 32, 32, 64], use_xyz=True),
            PointnetSAModule(npoint=384, radius=80.0, nsample=32,
                mlp=[64, 64, 64, 128], use_xyz=True),
            PointnetSAModule(npoint=256, radius=130.0, nsample=32,
