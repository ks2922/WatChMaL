...
class PointNet2MultiRingVoteNetV8TDAPerRing(nn.Module):
    """
    Same as PointNet2MultiRingVoteNetV8 (spatial clustering + cross-ring
    attention), with each ring's own precomputed, frozen persistence-image
    TDA feature concatenated into that ring's own head input -- matching
    V8's per-ring architecture rather than fusing one whole-event TDA
    blob into both heads identically. TDA features assigned via oracle
    (ground-truth vertex) hit-to-ring assignment at precompute time.
    ...
    def __init__(self, num_input_channels, num_rings=2, channels_per_ring=7,
                 ...
                 cross_ring_attn_heads=4, tda_dim=800, **kwargs):
        super().__init__()
        ...
        # TDA features projected down before concatenation, same principle
        # as the single-ring gated/cross-attention variants: don't feed the
        # raw 800-d persistence image directly into the head, give it a
        # small trainable adapter first
        self.tda_proj = nn.Sequential(
            nn.Linear(tda_dim, 64), nn.ReLU(),
        )

        head_in_dim = 512 + slot_embed_dim + 64  # cluster feat + slot embed + projected TDA
    ...
    def forward(self, x, tda_features_ring0=None, tda_features_ring1=None):
        ...
        for r in range(self.num_rings):
            ...
            main_in = torch.cat([cluster_feats[:, r], slot_emb, tda_proj_r], dim=1)
