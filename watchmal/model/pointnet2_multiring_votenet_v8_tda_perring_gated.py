...
class PointNet2MultiRingVoteNetV8TDAPerRingGated(nn.Module):
    """
    Same as PointNet2MultiRingVoteNetV8TDAPerRing, except each ring's
    projected TDA feature passes through a learned gate (conditioned on
    that ring's own cluster feature) before concatenation into the head --
    mirrors the single-ring gated-fusion design, giving the network an
    explicit per-event, per-ring mechanism to suppress TDA when unreliable,
    rather than a single fixed weighting learned by the head alone.
    ...
    def __init__(self, num_input_channels, num_rings=2, channels_per_ring=7,
                 ...
                 cross_ring_attn_heads=4, tda_dim=800, tda_proj_dim=64, **kwargs):
        super().__init__()
        ...
        self.tda_proj_dim = tda_proj_dim
        ...
        self.tda_proj = nn.Sequential(
            nn.Linear(tda_dim, tda_proj_dim), nn.ReLU(),
        )
        # gate conditioned on the ring's own (post-attention) cluster feature,
        # same principle as PointNet2TDAGated -- outputs a per-dimension
        # weight in (0,1) applied to the projected TDA feature
        self.tda_gate = nn.Sequential(
            nn.Linear(512, 128), nn.ReLU(),
            nn.Linear(128, tda_proj_dim), nn.Sigmoid(),
        )

        head_in_dim = 512 + slot_embed_dim + tda_proj_dim
    ...
    def forward(self, x, tda_features_ring0=None, tda_features_ring1=None):
        ...
        tda_per_ring = [tda_features_ring0, tda_features_ring1]

        main_outs, aux_outs = [], []
        for r in range(self.num_rings):
            slot_emb = self.slot_embeddings[r].unsqueeze(0).expand(global_feat.shape[0], -1)

            tda_proj_r = self.tda_proj(tda_per_ring[r])
            gate_r = self.tda_gate(cluster_feats[:, r])
            gated_tda_r = tda_proj_r * gate_r

            main_in = torch.cat([cluster_feats[:, r], slot_emb, gated_tda_r], dim=1)
