...
class PointNet2MultiRingVoteNetV8GroupNorm(nn.Module):
    """
    Same as PointNet2MultiRingVoteNetV8, with the FC head's BatchNorm1d
    layers replaced by GroupNorm -- tests whether the same fix that
    dramatically improved ResNet's multi-ring result (removing dependence
    on batch-level statistics) also helps PointNet++'s head.

    NOTE: only the head is changed here, not the SA-layer backbone --
    PointnetSAModule (third-party pointnet2_ops library) hardcodes
    BatchNorm2d with no configurable alternative, and patching that
    submodule directly was judged too invasive/risky given remaining
    project scope. This is therefore a partial test of the GroupNorm
    hypothesis, not a full replication of the ResNet fix.
    ...
    def __init__(self, num_input_channels, num_rings=2, channels_per_ring=7,
                 ...
                 cross_ring_attn_heads=4, n_groups=32, **kwargs):
        super().__init__()
        ...
        def gn(num_channels):
            groups = min(n_groups, num_channels)
        ...
        self.main_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(512 + slot_embed_dim, 256), gn(256), nn.ReLU(), nn.Dropout(dropout),
                nn.Linear(256, 128), gn(128), nn.ReLU(), nn.Dropout(dropout),
