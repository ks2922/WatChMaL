...
class PointNet2MultiRingVoteNetV3(nn.Module):
    """
    Same as PointNet2MultiRingVoteNetV2 (differentiable soft-assignment
    clustering), with one addition: forward now also returns vote_xyz and
    seed_xyz, so the training engine can add an explicit vote-position
    supervision loss (Deep Hough Voting style, Qi et al. 2019) -- each
    seed point's vote is directly supervised to move toward whichever
    true ring position is nearer, rather than relying entirely on the
    downstream regression loss to shape the votes indirectly.
...
def vote_supervision_loss(vote_xyz, true_positions):
    """
    Deep Hough Voting style loss: each seed's vote is supervised toward
    whichever true ring position is nearest to its CURRENT vote location
    (per-point pseudo-label, since we don't have per-hit ring-membership
    ground truth).

    vote_xyz: (B, 64, 3) -- model's predicted vote positions
    true_positions: (B, 2, 3) -- target_dict["positions"], RAW (unscaled) units
    """
    dists = torch.linalg.norm(
        vote_xyz.unsqueeze(2) - true_positions.unsqueeze(1), dim=-1
    )  # (B, 64, 2)

    nearest_ring = dists.argmin(dim=-1)  # (B, 64)
    target_pos = torch.gather(
        true_positions, 1,
        nearest_ring.unsqueeze(-1).expand(-1, -1, 3)
    )  # (B, 64, 3)

    return torch.nn.functional.smooth_l1_loss(vote_xyz, target_pos.detach())
