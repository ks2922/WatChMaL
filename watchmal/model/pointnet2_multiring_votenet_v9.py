...
class PointNet2MultiRingVoteNetV9(nn.Module):
    """
    Same as PointNet2MultiRingVoteNetV3, but main_heads are split by output
    quantity: position+energy (4 channels) are predicted from the
    spatially-clustered feature (where clustering helped in earlier
    experiments), while direction (3 channels) is predicted separately
    from the full global (unclustered) feature (where V2, without
    clustering, consistently outperformed every clustering variant).

    Output channel order is reassembled to match [position(3),
    direction(3), energy(1)], matching what the engine expects at indices
    0:3, 3:6, 6.
    ...
    def __init__(self, num_input_channels, num_rings=2, channels_per_ring=7,
        ...
        assert channels_per_ring == 7, "V9's split heads assume [position(3), direction(3), energy(1)]"
        ...
        # Position + energy: reads the spatially-clustered feature (4 out: xyz + energy)
        ...
        # Direction: reads the full global (unclustered) feature (3 out)
    ...
    def forward(self, x):
        ...
        for r in range(self.num_rings):
            ...
            pos_energy = self.pos_energy_heads[r](torch.cat([cluster_feats[:, r], slot_emb], dim=1))  # (B, 4)
            direction = self.direction_heads[r](torch.cat([global_feat, slot_emb], dim=1))  # (B, 3)

            # Reassemble to [position(3), direction(3), energy(1)]
            ring_out = torch.cat([pos_energy[:, 0:3], direction, pos_energy[:, 3:4]], dim=1)  # (B, 7)
            main_outs.append(ring_out)
