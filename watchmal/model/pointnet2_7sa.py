...
class PointNet2(nn.Module):
    """PointNet++ 7SA - physics motivated, conservative npoint reduction."""
    def __init__(self, num_input_channels, num_output_channels, dropout=0.3, **kwargs):
        ...
        self.SA_modules = nn.ModuleList([
            ...
            PointnetSAModule(npoint=384, radius=30.0, nsample=32,
                mlp=[64, 64, 64, 128], use_xyz=True),
            PointnetSAModule(npoint=256, radius=55.0, nsample=32,
                mlp=[128, 128, 128, 256], use_xyz=True),
            PointnetSAModule(npoint=128, radius=90.0, nsample=32,
                mlp=[256, 256, 256, 512], use_xyz=True),
            PointnetSAModule(npoint=64, radius=130.0, nsample=32,
                mlp=[512, 512, 512, 512], use_xyz=True),
            PointnetSAModule(npoint=32, radius=180.0, nsample=32,
                mlp=[512, 512, 512, 1024], use_xyz=True),
            PointnetSAModule(mlp=[1024, 512, 1024], use_xyz=True),
        ])
