...
class PointNet2MultiRingVoteNetV11(nn.Module):
    """
    Same as PointNet2MultiRingVoteNetV8 (spatial clustering + cross-ring
    attention between the two cluster features), except the output head is
    split by quantity, following V9's logic: position + energy (4 channels)
    are still predicted from the cross-ring-attended cluster feature (V8's
    strong pathway for position/momentum), while direction (3 channels) is
    predicted separately from the unclustered global feature -- the
    pathway that consistently gives the best direction resolution across
    V2 and V9, without going through clustering or cross-ring attention at
    all.

    Motivation: V10 (V6 seed-attention + V8) and V8+cost-norm both made
    things WORSE than V8 alone -- both modified something upstream of or
    coupled to V8's clustering/loss. This combination is deliberately
    minimally invasive: it does not touch seed-level processing (unlike
    V6) or the loss function (unlike cost-norm), only which already-
    computed feature feeds the direction head. Position/momentum pathway
    is untouched from V8.

    Output channels are reassembled to [position(3), direction(3),
    energy(1)] to match what the engine expects at indices 0:3, 3:6, 6.
    ...
    def __init__(self, num_input_channels, num_rings=2, channels_per_ring=7,
        ...
        # position(3) + energy(1) = 4 channels, from cross-ring-attended cluster feature
        self.pos_energy_heads = nn.ModuleList([
            nn.Sequential(
        ...
        # direction(3) channels, from unclustered global feature -- same
        # pathway V2/V9 use, deliberately not touched by clustering/attention
        self.direction_heads = nn.ModuleList([
            nn.Sequential(
                nn.Linear(1024 + slot_embed_dim, 256), nn.BatchNorm1d(256), nn.ReLU(), nn.Dropout(dropout),
                nn.Linear(256, 128), nn.BatchNorm1d(128), nn.ReLU(), nn.Dropout(dropout),
                nn.Linear(128, 3),
            ) for _ in range(num_rings)
        ])
    ...
    def forward(self, x):
        ...
        for r in range(self.num_rings):
            ...
            pos_energy_in = torch.cat([cluster_feats[:, r], slot_emb], dim=1)
            pos_energy = self.pos_energy_heads[r](pos_energy_in)  # (B, 4): [pos(3), energy(1)]

            direction_in = torch.cat([global_feat, slot_emb], dim=1)
            direction = self.direction_heads[r](direction_in)  # (B, 3)

            # reassemble to [position(3), direction(3), energy(1)]
            main_out_r = torch.cat([pos_energy[:, :3], direction, pos_energy[:, 3:]], dim=1)
            main_outs.append(main_out_r)
