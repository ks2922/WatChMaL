...
class PointNet2MultiRingVoteNetV8TDAMergeDistGated(nn.Module):
    """
    Same as PointNet2MultiRingVoteNetV8TDAMergeDist, except the merge-
    distance scalar passes through a learned gate before concatenation,
    conditioned on the ring's own cluster feature. Since merge distance is
    a single scalar (not an 800-d vector), the gate here is a single
    learned value per ring rather than per-dimension.
    ...
    def __init__(self, num_input_channels, num_rings=2, channels_per_ring=7,
        ...
        head_in_dim = 512 + slot_embed_dim + 1
    ...
    def forward(self, x, merge_distance=None):
        ...
        merge_dist = merge_distance.unsqueeze(-1)

        main_outs, aux_outs = [], []
        for r in range(self.num_rings):
            slot_emb = self.slot_embeddings[r].unsqueeze(0).expand(global_feat.shape[0], -1)

            gate_r = self.merge_gate(cluster_feats[:, r])
            gated_merge_r = merge_dist * gate_r

            main_in = torch.cat([cluster_feats[:, r], slot_emb, gated_merge_r], dim=1)
