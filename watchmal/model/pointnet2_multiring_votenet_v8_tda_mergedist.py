...
class PointNet2MultiRingVoteNetV8TDAMergeDist(nn.Module):
    """
    Same as PointNet2MultiRingVoteNetV8, with a single scalar "merge
    distance" TDA feature concatenated into both rings' head inputs.
    Merge distance is the radius at which the whole event's H0 diagram
    collapses to one connected component -- a cheap, interpretable proxy
    for how spatially separable the two rings are, computed once per
    event (not per ring, since it describes the whole event).
    ...
    def __init__(self, num_input_channels, num_rings=2, channels_per_ring=7,
        ...
        head_in_dim = 512 + slot_embed_dim + 1  # + merge distance scalar
    ...
    def forward(self, x, merge_distance=None):
